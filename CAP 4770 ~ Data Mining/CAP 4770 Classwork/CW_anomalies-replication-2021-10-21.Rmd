...
```{r}
plot(df)
points(df[outlier_list_1, ], col = "red", pch = "+", cex = 2.5)
```
...
```{r}
plot(df)
points(df[outlier_list_2, ], col = "blue", pch = "x", cex = 2)
```
...
# pick top 5 as outliers
outliers <- order(outliers_scores, decreasing = T)[1:5];outliers
```
...
kmeans_result <- kmeans(iris_2, centers = 3); kmeans_result$centers
