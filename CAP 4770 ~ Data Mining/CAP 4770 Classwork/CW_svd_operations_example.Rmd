...
where $\Sigma = \text{diag}(d)$ is a diagonal matrix containing the **singular values** of matrix $X$. Both matrices, $U$ and $V$ are **orthogonal**, that is, $U^TU$ and $V^TV$ are equal to the _identity matrix_ (of the associated dimensions)
...
```{r}
# confirming that U is an orthogonal matrix
t(myU) %*% myU
```
